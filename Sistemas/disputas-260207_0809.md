...
- Compatibilidad multi-país: policies versionadas por país (sin romper histórico).
...
- ScopeGuard (country_code en roles internos)


- PolicyGuard (ventana de disputa, outcomes habilitados, thresholds, caps)


- EvidenceGuard (solo muestra outcomes permitidos según evidencia/estado)


- MoneyGuard (prohíbe montos manuales; solo inputs controlados: banda/items)
...
- Buyer/Seller/Support abre disputa (reason_code) sobre order_id.


- Se bloquea la orden/escrow: **hold** (no payout/release) hasta resolución.
...
- DISPUTE_OPENED (lock order_id)


- OUTCOME_SELECTED


- OUTCOME_COMPUTED (buckets + settlement_plan)


- EXECUTE_REFUND (si aplica)


- EXECUTE_RELEASE/CLAWBACK (si aplica)


- LEDGER_ADJUSTMENTS (append-only)


- DOCS_ADJUSTMENTS (async)


- TRUST_LOYALTY_UPDATES (async)
...
- Orden ya tenía payout parcial: Saga aplica *clawback/reserve* según policy (si la plataforma soporta).
...
- “Montos manuales” → eliminado: solo templates + inputs controlados.


- Platform Fee “sagrado % fijo” → eliminado: earned schedule + fault + evidencia.


- Costos externos ignorados → eliminado: ExternalCosts bucket + shield.


- Ejecución frágil → eliminado: Saga idempotente por step + DLQ/retries.


- Falta de forensia → eliminado: WORM packet con snapshot+evidencias+ledger+docs.
