# support · Transcripción integral desde `sistema-de-soporte-260207_0731.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-soporte-260207_0731.docx`
- Dominio canónico asociado: `support`
- Título detectado: Sistema de Soporte v2.0 (Support OS + Disputas) — corregido y unificado
...
- Objetivos:
- Resolver 80% con automatización + procesos (L0) y evitar escalamiento innecesario.
- Separar claramente:
...
- “Regla de oro”: Soporte no inventa montos; selecciona un outcome y el sistema calcula.
- Evidencia manda: PoD (PIN+GPS+Foto) y timeline auditado para resolver objetivamente.
- Seguridad operativa: doble aprobación para reembolsos grandes/sensibles.
...
- PolicyGuard (ventanas, thresholds “large refund”, force-complete enabled, etc.)
- EvidenceGuard (para acciones que dependan de PoD)
- MoneyGuard (bloquea montos manuales; valida bucketización)
...
- Montos manuales por agentes → prohibido: outcomes + buckets + snapshot + cálculo determinístico.
- Overrides sin control → force_complete solo L3, con evidencia y auditoría; reembolsos grandes con doble aprobación.
- Decisión sin evidencia → caja negra + tridente + señales de riesgo como base objetiva.
- Ejecución financiera frágil → saga + idempotencia + WORM (audit legal).
