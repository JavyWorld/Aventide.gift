# orders · Transcripción integral desde `sistema-de-ordenes-260207_0037.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-ordenes-260207_0037.docx`
- Dominio canónico asociado: `orders`
- Título detectado: Sistema de Órdenes v2.0 (corregido y unificado)
...
- “Evidencia > opiniones”: disputas se resuelven con rastro (PIN+GPS+Foto+logs).
- “SLA medible”: evitar debates; todo se mide por deadlines.
...
- ScopeGuard (country/hub/zone para internos)
- PolicyGuard (reglas por país: ventanas, SLAs, allowed cancel reasons)
- StateGuard (validación de transición)
- MoneyGuard (escrow/release y antifraude)
- EvidenceGuard (requisitos PoD para cerrar)
...
- Cancelar “en cualquier momento” → corregido: pre-aceptación libre; post-aceptación crea caso y outcome.
- Completar sin evidencia → corregido: DELIVERED_PENDING_RELEASE exige PoD válido para COMPLETED y release de escrow.
- Estado y refund mezclados → corregido: máquina de refund separada acoplada a orden.
- SLA “de palabra” → corregido: deadlines persistidos + timers + eventos de breach.
- Editar el pasado → corregido: snapshots inmutables + eventos compensatorios.
