# memory · Transcripción integral desde `sistema-de-memory-260207_1012.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-memory-260207_1012.docx`
- Dominio canónico asociado: `memory`
- Título detectado: Sistema Memory v2.0 (Gift Memory OS) — Solo Memory (sin Genie)
...
- Consistencia: una única fuente de verdad para preferencias del destinatario.
- Privacidad: controlar “qué ve el seller” vs “qué guarda el buyer” y soportar “Admirador Secreto” como política transversal.
...
- 3.3 Guards
- AuthGuard
- OwnershipGuard: buyer solo accede a recipients propios.
...
- Happy path
- Buyer crea recipient: nombre/alias, relación (pareja, mamá, amigo), país/ciudad opcional, zona horaria.
- Sistema genera recipient_id scoped al buyer (no global).
- Se crea un memory_profile vacío con defaults.
- Edge cases
...
- allergies (si aplica),
- notes_private.
- Se valida contra taxonomía por país (evita tags sucios).
...
- Happy path
- En ORDER_COMPLETED, si el buyer asignó un recipient a la orden:
...
- Si hay REFUND/RETURN: se registra outcome para no repetir.
- Edge cases
...
- Happy path
- Buyer guarda fecha (cumpleaños/aniversario).
- Configura recordatorios (p. ej. 14 días antes, 3 días antes).
- Scheduler emite REMINDER_DUE y el sistema de Notificaciones lo entrega (Inbox como verdad).
- Edge cases
...
- Memory como recomendador → corregido: Memory solo almacena preferencias/señales; recomendación es Genie/Search.
- Compartición accidental con sellers → corregido: privado por defecto + whitelist de campos shareable.
- Recipients globales (riesgo de privacidad) → corregido: recipients son scoped al buyer.
- Datos sensibles sin control → corregido: DataClassGuard + VIEW_SENSITIVE + auditoría WORM.
- Señales automáticas sin destinatario (inferencias peligrosas) → corregido: ingesta post-compra solo si el buyer asignó recipient; si no, no se infiere.
- Recordatorios sin timezone/política multi-país → corregido: timezone gobernado por país/hub/recipient y disparo determinista.
