# coupons · Transcripción integral desde `sistema-de-cupones-260207_0826.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-cupones-260207_0826.docx`
- Dominio canónico asociado: `coupons`
- Título detectado: Sistema de Cupones v2.0 (Seller-funded Coupons) — corregido y unificado
...
- Objetivos:
- Coherencia financiera: el cupón reduce el subtotal base del seller y sobre ese neto se calculan fees posteriores.
...
- Anti-abuso operativo: rate limit y señales obligatorias para first-time buyer-only.
...
- ScopeGuard (country/hub/zone) para enforcement ops
- PolicyGuard (reglas del país/territorio)
- RateLimitGuard (apply attempts)
...
- Happy path
- Buyer ingresa código en checkout.
...
- stacking_policy.
- Backend calcula seller_coupon_discount (si % respeta max_discount_amount).
- Recalcula breakdown siguiendo el orden oficial (ver 5).
...
- Se marca coupon_redemption como CONSUMED.
- Edge cases
...
- delivery
- taxes
- ops lead fee (no-discountable)
- processing fee (no-discountable)
- platform fee
- membership benefits (si aplica)
- Fee Shields (solo contra platform fee)
...
- “¿Quién paga el cupón?” → definido duro: siempre seller-funded; baja net del seller.
- Orden de cálculo inconsistente (rompe docs/ledger/BI) → fijado por Torre de Precios.
- % OFF sin cap → obligatorio max_discount_amount y respeto en backend.
- Aplicación duplicada por reintentos → idempotencia por checkout_id + consumo por order_id.
- Abuso de cupones → rate limit + señales first-time buyer + escalación a Trust/Moderation.
