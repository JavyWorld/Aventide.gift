# notifications · Transcripción integral desde `sistema-de-notificaciones-260207_0929.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-notificaciones-260207_0929.docx`
- Dominio canónico asociado: `notifications`
- Título detectado: Sistema de Notificaciones v2.0 (Notification Center + Inbox) — corregido y unificado
...
- Anti-spam: rate limits + batching + dedupe.
- Multi-país: plantillas, idioma, legal footers y quiet hours por región/hub y por rol.
...
- 3.3 Guards
- AuthGuard
- OwnershipGuard (inbox/preferencias)
- ScopeGuard (plantillas por país/hub; staff)
- PolicyGuard (quiet hours, severidad, canal permitido, legales)
- DedupeGuard (dedupe_key)
- RateLimitGuard (por canal/rol)
...
- En la misma transacción escribe un registro en outbox_events (event_id, event_type, payload).
- Worker consume outbox_events → llama al Orchestrator.
- Orchestrator:
...
- Happy path
- Mensajería emite NEW_CHAT_MESSAGE.
- Notification Center aplica reglas:
...
- Push/Email como fuente de verdad → corregido: Inbox es la verdad; canales externos son best-effort.
- Pérdida de notificaciones por fallos transaccionales → corregido: Outbox pattern dentro de la transacción + worker consumidor.
- Duplicados por reintentos → corregido: dedupe_key event_id + template_id + recipient_id.
- Soporte marca disputa “resuelta” aunque el refund falló → corregido: DISPUTE_STEP_FAILED P0 a soporte y no se dispara resolved hasta completar la saga.
- Plantillas editables sin barandas → corregido: registry versionado + whitelist variables + bloqueo P0 + preview/simulador + rollback.
