# continuity · Transcripción integral desde `sistema-de-continuidad-v2-260207_1030.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-continuidad-v2-260207_1030.docx`
- Dominio canónico asociado: `continuity`
- Título detectado: Sistema de Continuidad v2.0 (Solo Continuidad) — “Role Continuity & Country Governance”
...
- En vacancia, el bucket “ops” no desaparece: se enruta a COUNTRY_RESERVE con custodia corporativa, no “backpay” al nuevo COL.
...
- country_governance_state = VACANT.
- Policy Engine activa “modo continuidad” para ese país:
...
- Happy path
- Crear ops_lead_assignment nuevo con effective_from.
- (Recomendación operativa del sistema) four-eyes: requester ≠ approver para asignación de COL.
- Grant de COUNTRY_OPS_LEAD con scope del país.
- Reroute de colas al nuevo COL.
- country_governance_state = ACTIVE.
...
- Happy path
- ops_lead_assignment.status = TEMP_UNAVAILABLE con until.
- Conceder role_delegation a un ActingOpsLead con TTL y subset de permisos.
- Si no hay delegado, se eleva a SUPER_ADMIN_QUEUE (cobertura global).
- Edge case
...
- Happy path
- Policy Engine coloca country_governance_state = LOCKDOWN.
- Efectos:
...
- Alertas internas P0/P1 a Global Management.
...
- No retroactividad de dinero: toda orden usa locked_fee_structure y el beneficiario del bucket ops se define al momento del snapshot.
- VACANT ≠ “sin ops fee”: el bucket ops se enruta a COUNTRY_RESERVE hasta nuevo aviso.
- Break-glass para acciones críticas: 2FA + motivo + TTL + auditoría reforzada.
- Four-eyes obligatorio para: asignación/remoción de COL y disbursements de COUNTRY_RESERVE.
...
- Revocar rol y scope del COL saliente + invalidar sesiones/tokens.
- Evidencia WORM: RBAC_ROLE_REVOKED, RBAC_SCOPE_REVOKED, SESSION_INVALIDATED.
- Set country_governance_state=VACANT con reason_code.
- Evidencia WORM: COUNTRY_GOVERNANCE_STATE_CHANGED.
- Activar ContinuityPolicyGuard (automático por policy):
...
- Comunicar internamente: notificación P0/P1 a oncall global + finance + audit.
...
- Crear role_delegation a ACTING_COUNTRY_OPS_LEAD con TTL y subset de permisos (mínimo viable).
- Evidencia WORM: ROLE_DELEGATION_GRANTED (TTL, permisos, motivo).
- Set country_governance_state=TRANSITION y asociar acting_user_id.
...
- Acciones mínimas
- Timeline WORM: extraer y fijar un “incident bundle” con:
...
- Resultado: clasificar severidad y decidir:
...
- Set country_governance_state=ACTIVE.
- Reroute colas al COL entrante.
- Activar “Monitoreo reforzado post-reinstatement” por 7–14 días (policy).
...
- Acciones (inmediatas)
- Set country_governance_state=LOCKDOWN con reason_code.
- Efectos (Policy Engine):
- bloquear onboarding approvals / cambios masivos,
- requerir SUPER_ADMIN para acciones críticas,
...
- Notificación P0 a Global Management.
