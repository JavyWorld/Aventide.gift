# messaging · Transcripción integral desde `sistema-de-mensajeria-260207_0925.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-mensajeria-260207_0925.docx`
- Dominio canónico asociado: `messaging`
- Título detectado: Sistema de Mensajería v2.0 (Chat in-app) — corregido y unificado
...
- AuthGuard
- OrderScopeGuard: conversación solo existe si hay orden válida y el actor es buyer o seller de esa orden.
- StatusGuard: si Conversation.status=FROZEN ⇒ solo lectura; solo SYSTEM/SUPPORT pueden emitir SYSTEM_EVENT.
- ModerationGuard (síncrono) antes de persistir/entregar.
- RateLimitGuard (por conversación y por usuario).
...
- Happy path
- POST /conversations/:id/messages con type y payload.
- Moderación síncrona:
- ALLOW → persistir.
...
- Persistir mensaje (inmutable) y emitir NEW_CHAT_MESSAGE a Notification Center.
- Entrega:
...
- Happy path
- Buyer o seller presiona “Reportar Problema”.
- Se crea SupportTicket(order_id, conversation_id) y se adjunta historial completo como evidencia.
- Conversation.status = FROZEN (solo lectura).
- Se notifica a Soporte (y a Country Ops Lead si aplica).
- Edge cases
...
- Edición/borrado de mensajes (rompe evidencia) → prohibido: mensajes inmutables.
- PII y off-platform sin control → corregido: moderación síncrona + redacción/bloqueo + rate limit + escalamiento a Trust.
- Manipulación durante disputa → corregido: “Reportar Problema” crea ticket y congela conversación (FROZEN) para preservar evidencia.
