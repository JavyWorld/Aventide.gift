# reputation-buyer · Transcripción integral desde `sistema-de-reputacion-buyer-260207_0839.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-reputacion-buyer-260207_0839.docx`
- Dominio canónico asociado: `reputation-buyer`
- Título detectado: Sistema de Reputación v2.0 (Buyer Only) — corregido y unificado
...
- Guards (runtime)
- AuthGuard
- ContextGuard (seller solo ve trust de buyer si hay relación de orden o pre-check de checkout)
- ScopeGuard (ops/admin por país)
- PolicyGuard (thresholds y fricción por país/ciudad)
...
- Trust público (exposición del buyer) → corregido: semi-visible al seller como nivel+badge, no score público.
- Score sin base verificable → corregido: solo eventos auditables (COMPLETED, disputes, cancelaciones, chat flags, moderation).
- Fricción aplicada “a mano” → corregido: fricción deriva de policy flags consumidos por checkout/órdenes.
- Review bombing sin defensa → corregido: integridad de reviews como subscore + holds/penalizaciones.
- Admirador Secreto sin señal de seguridad → corregido: Trust Badge visible al seller aun en anonimato.
