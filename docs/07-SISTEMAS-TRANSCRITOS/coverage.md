# coverage · Transcripción integral desde `sistema-de-cobertura-260207_0907.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-cobertura-260207_0907.docx`
- Dominio canónico asociado: `coverage`
- Título detectado: Sistema de Cobertura v2.0 (Coverage & Logistics Engine) — corregido y unificado
...
- AuthGuard
- OwnershipGuard (address_id para buyer / seller_id para seller)
- TerritoryGuard (resolve country/hub/zone y valida que el hub esté activo)
- AddressGuard: sin lat/long validado → FAIL
- FulfillmentGuard: ruta por fulfillment_type (HYPER_LOCAL vs WORLDWIDE)
- AuditGuard: registra coverage_check_run con versión algoritmo y resultado.
- 4) Flujos end-to-end (happy path + edge cases)
- 4.1 Address Intelligence (validate → save)
- Happy path
- Buyer usa POST /addresses/validate con raw_address.
- Sistema llama proveedor (Google Places/Mapbox) y devuelve:
- normalized_address
- location (lat/long)
- country_code
- address_quality_score
- geocode_provider_meta (place_id, confidence)
- Buyer confirma y guarda con POST /addresses.
...
- Happy path
- GET /coverage/check?seller_id&address_id&date_time
- Valida address con lat/long.
...
- Happy path
- No evalúa radio.
- Valida destination_country ∈ seller.shipping_countries.
...
- Guardar direcciones sin lat/long → prohibido (sin coordenadas no hay cobertura).
- Mostrar productos “comprables” que luego fallan en checkout → corregido: Búsqueda usa ST_DWithin y Coverage usa el mismo motor/radio.
- Coverage evaluada después de slots/pagos → corregido: Coverage Guard corre antes de scheduler y antes de dinero.
- Direcciones caóticas sin evidencia para soporte → corregido: Address Intelligence guarda place_id/confidence + snapshot logístico por orden.
