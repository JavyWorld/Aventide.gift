# backup · Transcripción integral desde `sistema-de-copia-de-seguridad-260207_0955.docx`

> Este archivo es una transcripción documental completa del `.docx` histórico para lectura IA/humana en formato Markdown.

## Metadatos

- Fuente original: `Sistemas/sistema-de-copia-de-seguridad-260207_0955.docx`
- Dominio canónico asociado: `backup`
- Título detectado: Sistema de Copia de Seguridad v2.0 (Backups + Continuidad + DR) — corregido y unificado
...
- Inmutabilidad de evidencia fiscal/legal (WORM) + replicación sin romper residencia de datos.
...
- 3.3 Guards
- AuthGuard
- ScopeGuard (país/cluster)
- BreakGlassGuard (dual approval + logging inmutable)
- KeyGuard (no restore sin llaves correctas; separación de llaves por clase)
- WORMGuard (documentos emitidos no modificables/no borrables antes de retención)
- AuditGuard (restore/export/lecturas bóveda y evidencias)
- 4) Flujos end-to-end (happy path + edge cases)
- 4.1 Backups OLTP (PITR + snapshots + export lógico)
- Happy path
- PITR continuo: WAL/journal se archiva a storage seguro.
- Snapshot físico diario.
- Export lógico semanal (SQL/Parquet) como último recurso multi-cloud.
- Validación automática: checksum + verificación de consistencia post-backup.
- Edge cases
...
- Reconciliation job diario/horario compara:
...
- Al restaurar Tier 0:
- Plataforma en “read-only de dinero”: permitir navegar/soporte, bloquear:
- nuevos payouts
- refunds automáticos
- cambios de fee/tax
...
- “Backup = continuidad” ignorando WORM → corregido: WORM es la caja fuerte; continuidad se logra con replicación + manifests; backups clásicos no sustituyen.
- Restores que duplican dinero → corregido: Tier 0 exige restauración idempotente + “modo read-only de dinero” + reconciliación con proveedor.
- Retención única para todo (rompe compliance) → corregido: retención diferenciada por clase + policy por país.
- GDPR/privacy destruyendo evidencia financiera → corregido: DELETED_PENDING_ARCHIVE + bóveda fiscal (cold) por tiempo legal, luego purga.
- Backups “sin pruebas” → corregido: backup.restore_smoketest + gating post-restore + métricas/alertas obligatorias.
